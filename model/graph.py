...
class Node:
    def __init__(self, id, x, y, demand, from_time, to_time, service_time):
        super()
        self.id = id
        ...
        self.y = y
        self.demand = demand
        self.ready_time = from_time
        self.due_time = to_time
...
class Graph:
    def __init__(self, file_path, rho=0.1):
        ...
        self.nnh_travel_path, self.init_pheromone_val, _ = self.nearest_neighbor()
    ...
    def get_data_from_file(file_path):
        ...
        nodes = list(Node(int(item[0]), float(item[1]), float(item[2]), float(item[3]), float(item[4]),
                          float(item[5]), float(item[6])) for item in node_list)
    ...
    def nearest_neighbor(self, max_vehicle_num=None):
        ...
        while len(index_to_visit) > 0 and max_vehicle_num > 0:
            nearest_next_index = self.cal_nearest_next_index(index_to_visit, current_index, current_load, current_time)
        ...
        # in the end go back to the depot
        travel_distance += self.node_dist_mat[current_index][0]
        travel_path.append(0)
    ...
    def cal_nearest_next_index(self, index_to_visit, current_index, current_load, current_time):
        nearest_ind = None
        ...
        for next_index in index_to_visit:
            ...
            # check if you can return to the depot after visiting a customer
            if current_time + dist + wait_time + service_time + self.node_dist_mat[next_index][0] > self.nodes[0].due_time:
                continue

            # cannot serve customers outside time windows
